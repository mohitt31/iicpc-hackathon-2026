...
wrong — **Coordinated Omission** — the centrepiece.
...
```
      ...
      │ (Firecracker) │   SBE binary    │  + reference     │   scored      │  (browser,   │
      │  isolates the │   wire protocol │    engine        │   deltas      │ display-only)│
      │  contestant   │                 │  + HDR telemetry │   (WS / SSE)  │              │
      └───────────────┘                 └──────────────────┘              └──────────────┘
            T1                                  T2                                T3
                    all three compile against contracts/interface_contract_v1.h (FROZEN v1.1)
```
...
1. Contestant submits an engine (source tarball or ELF) → **Sandbox** hashes it
...
2. **Bot Fleet** drives **constant-arrival-rate** load at the engine, tagging each
   order with its *intended* send time, recording **two** HDR histograms (naive +
...
3. Per-second HDR snapshots + correctness diffs flow to the **Telemetry Gateway**,
   which scores and ranks per Interface Contract §5 — reading p99 from the
   **merged additive HDR**, never an average.
4. The gateway diffs each tick and fans out **display-ready deltas** over
   WebSocket.
...
| Bot → Telemetry | **Per-second CSV HDR snapshots** (file) / HDR blobs | Mergeable raw distributions, not summaries; cold path |
| Telemetry → Leaderboard | **WebSocket JSON deltas** | Display layer; human-scale cadence (1–5 s); diff-only payloads |
...
```
INTAKE ──▶ BUILD ──▶ ATTEST ──▶ PACK ──▶ ORCHESTRATE (run microVM)
```

| Stage | File | What it does | Key decision |
|---|---|---|---|
| **Intake** | `intake/intake.sh` | sha256 → immutable `submission_id`; 50 MB cap; **lint-heuristic** banned-syscall scan; extract | The source scan is explicitly a *lint heuristic, not a security boundary* — it's defeated by `syscall(2)` by number, inline asm, `dlopen`. We documented this honestly in-code. |
| **Build** | `builder/` | Hermetic Docker build, **`--network none`**, static link, `-march=x86-64-v2` | Build isolation = filesystem/network → containers are the *right* tool here (unlike runtime). No network = no supply-chain fetch at build time. |
...
1. **Single-writer per symbol** — one thread mutates one book; no locks on the
   match path.
2. **Deterministic** — same input → byte-identical output, every run. Critical
   detail: `by_id_` (the `unordered_map` for O(1) cancel lookup) is **never
   iterated**, because hash iteration order is implementation-defined and would
   break determinism. Find/insert/erase only.
...
|---|---|---|
| Dense `vector<PriceLevel>` over price range `[1, 1e6]` | O(1) price→level; cache-friendly | Memory for the full ladder — fine for a benchmark with a bounded tick range |
...
| `unordered_map<seq → {price_idx, list_iterator}>` | O(1) cancel | Must never be iterated (determinism) — enforced & documented |
...
## 7. LLD — Component 2b: Bot Fleet + Telemetry (`bot.cpp`) — the centrepiece
...
asserted. (See §11.4 for the verified 35× gap.)

### 7.2 Honest measurement under load

| Technique | What & why |
|---|---|
| **CPU pinning** (`pthread_setaffinity_np`, skip core 0) | Without it the scheduler ping-pongs workers across cores → ~1 ms preemption jitter pollutes p99. With it, only *real* stalls are visible. macOS lacks this API — the bot detects it and degrades gracefully (documented). |
...
- `recv() == 0` (clean peer close) detected → no 100%-CPU infinite spin if an
  engine dies.
...
- **Integrity gate → exclusion before ranking.** Submissions whose self-test p99
...
p50 → p90 → p99 → p99.9 → p99.99 → max, **in nanoseconds**. No mean-latency
...
|---|---|---|---|---|
| 1 | Measurement methodology | **CO-corrected (Tene/Snyder)** | Naive round-trip histograms | Naive numbers are wrong by 35–80× under stalls; they'd rank engines incorrectly |
| 2 | Submission isolation (runtime) | **Firecracker microVM** | Docker container | Container shares host scheduler → pollutes the *latency* being measured |
...
| 11 | Price representation | **`int64` ticks** | `double` | Float is non-deterministic → breaks byte-exact replay |
...
| **Aeron / Chronicle transport** | Our topology is N-bots → 1-engine fan-in, not 1-publisher → many-subscribers. Wrong tool. |
...
- **Bot fleet = bare-metal `DaemonSet`**, `hostNetwork: true` (skip CNI overlay on
  ...
  **Guaranteed QoS** (`requests == limits`, integer CPU → exclusive cores via the
...
`cd bot-engine/build && ./test_order_book` → `=== Results: 19/19 passed ===`
...
`./demo/run_demo.sh` → `DIVERGE @ byte 244` pinpointing the LIFO price-time
...
`→ IDENTICAL: 12,913,116 bytes match`. (100k sent = 100k acked, 155,218 fills,
...
### 11.4 The centrepiece — Coordinated Omission proof (600k orders)
...
- **ASan + UBSan**: full suite rebuilt with sanitizers, re-run end-to-end →
  **0 findings** (the hot path allocates nothing after startup, so there's nothing
  to leak — now proven, not claimed).
- **32 bots on 4 cores** (8× oversubscription): `Sent=398,369 Acked=398,369`,
...
and CO-corrected percentiles agree within 1.1× at every rank — exactly how a
...
1. **Telemetry gateway in Rust/Go** reading VictoriaMetrics + additive-HDR blobs
   (same JSON contract → frontend unchanged).
...
# The centrepiece: Coordinated Omission proof
